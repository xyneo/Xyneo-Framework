...
(function(jQuery) {
  ...
  function getScriptPath() {
    ...
    return (path.split('/').length > 0) ? path.split('/').slice(0, -1)
        .join('/')
        + '/' : '';
  }
  var scriptpath = getScriptPath();

  // derived by Paul Irish https://gist.github.com/paulirish/1579671 - thanks
  // for your code!
  ...
  var clsMutationObserver = window.MutationObserver
      || window.WebKitMutationObserver || false;
  ...
  var getBrowserDetection = function() {
    ...
    d.isieold = (d.isie && !("msInterpolationMode" in domtest.style)); // IE6
    // and
    // older
    ...
    d.isie9 = d.isie && ("performance" in window)
        && (document.documentMode >= 9);
    d.isie10 = d.isie && ("performance" in window)
    ...
    d.isie7mobile = (!d.isie9mobile && d.isie7)
        && /iemobile/i.test(navigator.userAgent); // wp 7.0
    ...
    d.ischrome26 = (d.ischrome && ("transition" in domtest.style)); // issue
    // with
    // transform
    // detection
    // (maintain
    ...
    d.hasmstouch = (window.navigator.msPointerEnabled || false); // IE10+
    // pointer
    // events
    ...
    var check = [
        'transform', 'msTransform', 'webkitTransform', 'MozTransform',
        'OTransform' ];
    ...
    function detectCursorGrab() {
      ...
      if ((d.ischrome && !d.ischrome22) || d.isie)
        lst = []; // force setting for IE returns false positive and chrome
      // cursor bug
      ...
      return 'url(http://www.google.com/intl/en_ALL/mapfiles/openhand.cur),n-resize'; // thank
      // you
      // google
      // for
      // custom
  ...
  var NiceScrollClass = function(myopt, me) {
    ...
    this.ispage = /BODY|HTML/.test((self.opt.win) ? self.opt.win[0].nodeName
        : this.doc[0].nodeName);
    ...
    this.scrollTop = function(val) {
      return (typeof val == "undefined") ? self.getScrollTop() : self
          .setScrollTop(val);
    };

    this.scrollLeft = function(val) {
      return (typeof val == "undefined") ? self.getScrollLeft() : self
          .setScrollLeft(val);
    };
    ...
    if (this.ishwscroll) {
      ...
      if (cap.hastranslate3d && cap.isios)
        this.doc.css("-webkit-backface-visibility", "hidden"); // prevent
      // flickering
      ...
      function getMatrixValues() {
        ...
        if (tr && (tr.substr(0, 6) == "matrix")) { return tr.replace(
            /^.*\((.*)\)$/g, "$1").replace(/px/g, '').split(/, +/); }
        return false;
      }
      ...
      this.getScrollTop = function(last) {
        if (!last) {
          ...
          if (mtx)
            return (mtx.length == 16) ? -mtx[13] : -mtx[5]; // matrix3d 16 on
          // IE10
      ...
      this.getScrollLeft = function(last) {
        if (!last) {
          var mtx = getMatrixValues();
          if (mtx)
            return (mtx.length == 16) ? -mtx[12] : -mtx[4]; // matrix3d 16 on
          // IE10
      ...
      if (cap.hastranslate3d && self.opt.enabletranslate3d) {
        this.setScrollTop = function(val, silent) {
          ...
          self.doc.css(cap.trstyle, "translate3d(" + self.doc.translate.tx
              + "," + self.doc.translate.ty + ",0px)");
          if (!silent)
        ...
        this.setScrollTop = function(val, silent) {
          self.doc.translate.y = val;
          self.doc.translate.ty = (val * -1) + "px";
          self.doc.css(cap.trstyle, "translate(" + self.doc.translate.tx + ","
    ...
    function getWidthToPixel(dom, prop, chkheight) {
      ...
      if (isNaN(px)) {
        ...
        var brd = (px == 3) ? ((chkheight) ? (self.win.outerHeight() - self.win
            .innerHeight()) : (self.win.outerWidth() - self.win.innerWidth()))
            : 1; // DON'T TRUST CSS
        if (self.isie8 && px)
          px += 1;
        return (brd) ? px : 0;
    ...
    this.updateScrollBar = function(len) {
      if (self.ishwscroll) {
        ...
        pos.left += (self.rail.align) ? self.win.outerWidth()
            - getWidthToPixel(self.win, 'border-right-width') - self.rail.width
        ...
        if (self.railh && !self.locked) {
          ...
          var y = (self.railh.align) ? pos.top
              + getWidthToPixel(self.win, 'border-top-width', true)
              + self.win.innerHeight() - self.railh.height : pos.top
              + getWidthToPixel(self.win, 'border-top-width', true);
    ...
    this.init = function() {
      ...
      if (self.isie && self.zindex == 0 && self.opt.zindex == "auto") { // fix
        // IE
        // auto
        // == 0
        self.zindex = "auto";
      ...
      /*
       * self.ispage = true; self.haswrapper = true; // self.win = $(window);
       * self.docscroll = $("body"); // self.doc = $("body");
       */
      ...
      if (!self.ispage || (!cap.cantouch && !cap.isieold && !cap.isie9mobile)) {
        ...
        rail.width = Math.max(parseFloat(self.opt.cursorwidth), cursor
            .outerWidth())
        ...
        if (self.opt.horizrailenabled) {
          ...
          railh.height = Math.max(parseFloat(self.opt.cursorwidth), cursor
              .outerHeight());
          railh.css({
        ...
        if (self.ispage) {
          ...
          if (cap.isie && self.opt.disableoutline)
            self.win.attr("hideFocus", "true"); // IE, prevent dotted rectangle
          // on focused div
        ...
        if (cap.isie9mobile) {
          ...
          /*
           * var trace = function(msg) { var db = $("#debug"); if
           * (isNaN(msg)&&(typeof msg != "string")) { var x = []; for(var a in
           * msg) { x.push(a+":"+msg[a]); } msg ="{"+x.join(",")+"}"; } if
           * (db.children().length>0) { db.children().eq(0).before("<div>"+msg+"</div>"); }
           * else { db.append("<div>"+msg+"</div>"); } } window.onerror =
           * function(msg,url,ln) { trace("ERR: "+msg+" at "+ln); }
           */
          ...
          self.onmangotouch = function(e) {
            ...
            if ((py == self.scrollmom.lastscrolly)
            ...
            if (((tm - self.mangotouch.tm) > 80)
                || (self.mangotouch.dry != dry) || (self.mangotouch.drx != drx)) {
              ...
              self.scrollmom.stop();
              self.scrollmom.update(self.mangotouch.sx - dfx,
              ...
              var ds = Math.max(Math.abs(self.mangotouch.ly - py), Math
                  .abs(self.mangotouch.lx - px));
              self.mangotouch.ly = py;
              self.mangotouch.lx = px;
          ...
          if (cap.cantouch || self.istouchcapable || self.opt.touchbehavior
              || cap.hasmstouch) {
            ...
            self.ontouchstart = function(e) {
              ...
              if (!self.locked) {
                ...
                if (tg) {
                  var skp = (/INPUT/i.test(tg.nodeName))
                ...
                if (!cap.cantouch && !this.istouchcapable && !cap.hasmstouch) {
                  ...
                  var ip = (tg) ? /INPUT|SELECT|TEXTAREA/i.test(tg.nodeName)
                      : false;
                  if (!ip) {
            ...
            self.ontouchmove = function(e, byiframe) {
              ...
              if (self.rail.drag && (self.rail.drag.pt == 2)) {
                if (cap.cantouch && (typeof e.original == "undefined"))
                  return true; // prevent ios "ghost" events by clickable
                // elements
                ...
                if (self.railh && self.railh.scrollable) {
                  ...
                  if (self.ishwscroll && self.opt.bouncescroll) {
                    if (nx < 0) {
                      ...
                      nx = self.page.maxw
                          + Math.round((nx - self.page.maxw) / 2);
                      // fx = 0;
                    }
                  } else {
                    if (nx < 0) {
                ...
                if (self.rail.drag.dl) {
                  ...
                  if (self.rail.drag.ck == "v") {
                    if (ay > dz && (ax <= (ay * 0.3))) {
                      ...
                      self.rail.drag.dl = "f";
                      $("body").scrollTop($("body").scrollTop()); // stop iOS
                      // native
                      // scrolling
                      // (when
                      // active
                    ...
                    if (ax > dz && (ay <= (az * 0.3))) {
                      ...
                      self.rail.drag.dl = "f";
                      $("body").scrollLeft($("body").scrollLeft()); // stop iOS
                      // native
                      // scrolling
                      // (when
                      // active
          ...
          self.onmousemove = function(e) {
            if (self.rail.drag) {
              ...
              if (self.rail.drag.hr) {
                self.scroll.x = self.rail.drag.sx
                ...
                self.scroll.y = self.rail.drag.sy
              ...
              self.synched('mousemove', function() {
                if (self.rail.drag && (self.rail.drag.pt == 1)) {
                  self.showCursor();
                  if (self.rail.drag.hr)
                    self.doScrollLeft(Math.round(self.scroll.x
                        * self.scrollratio.x), self.opt.cursordragspeed);
                  else
                    self.doScrollTop(Math.round(self.scroll.y
                        * self.scrollratio.y), self.opt.cursordragspeed);
                }
              });
          ...
          if (cap.cantouch || self.opt.touchbehavior) {
            ...
            self.bind(self.win, "mousedown", self.ontouchstart); // control
            // content
            // dragging
          ...
          if (self.opt.cursordragontouch || !cap.cantouch
          ...
          if (!cap.cantouch && !self.opt.touchbehavior) {
            ...
            self.bind((cap.hasmousecapture) ? self.win : document, "mouseup",
                self.onmouseup);
          ...
          if (self.opt.enablemousewheel) {
            if (!self.isiframe)
              self.bind((cap.isie && self.ispage) ? document : self.docscroll,
        ...
        self.onkeypress = function(e) {
          ...
          if (self.hasfocus || (self.hasmousefocus && !domfocus)
            ...
            case 63234: // safari
              if (self.railh) {
                (ctrl) ? self.doScrollLeft(self.page.maxw) : self
                    .doScrollLeftBy(-24 * 3);
                ret = true;
              }
              break;
            ...
            case 36:
            case 63273: // safari
              (self.railh && ctrl) ? self.doScrollPos(0, 0) : self
                  .doScrollTo(0);
              ret = true;
              break;
            case 35:
            case 63275: // safari
              (self.railh && ctrl) ? self.doScrollPos(self.page.maxw,
                  self.page.maxh) : self.doScrollTo(self.page.maxh);
              ret = true;
              break;
            case 32:
              if (self.opt.spacebarenabled) {
                (shift) ? self.doScrollBy(self.view.h) : self
                    .doScrollBy(-self.view.h);
                ret = true;
              }
              break;
            case 27: // ESC
        ...
        if (self.opt.enablekeyboard)
          self.bind(document, (cap.isopera && !cap.isopera12) ? "keypress"
        ...
        if (!self.ispage && !self.haswrapper) {
          ...
          if (clsMutationObserver !== false) {
            ...
            self.bind(self.win, (cap.isie && !cap.isie9) ? "propertychange"
      ...
      if (this.doc[0].nodeName == 'IFRAME') {
        function oniframeload(e) {
          self.iframexd = false;
          try {
            var doc = 'contentDocument' in this ? this.contentDocument
          ...
          if (self.opt.enablekeyboard)
            self.bind(doc, (cap.isopera) ? "keypress" : "keydown",
                self.onkeypress);
    ...
    this.getContentSize = (self.ispage) ? function() {
      ...
      return {
        w : self.doc.outerWidth() + parseInt(self.win.css('paddingLeft'))
            + parseInt(self.win.css('paddingRight')),
        h : self.doc.outerHeight() + parseInt(self.win.css('paddingTop'))
    ...
    this.onResize = function(e, page) {
      ...
      self.view = {
        w : (self.ispage) ? self.win.width()
            : parseInt(self.win[0].clientWidth),
        h : (self.ispage) ? self.win.height()
            : parseInt(self.win[0].clientHeight)
      };
      ...
      if ((self.page.maxh == premaxh) && (self.page.maxw == premaxw)
      ...
      if (self.istextarea && self.win.css('resize')
          && self.win.css('resize') != 'none')
        self.view.h -= 20;

      self.cursorheight = Math.min(self.view.h, Math.round(self.view.h
          * (self.view.h / self.page.h)));
      self.cursorheight = (self.opt.cursorfixedheight) ? self.opt.cursorfixedheight
          : Math.max(self.opt.cursorminheight, self.cursorheight);

      self.cursorwidth = Math.min(self.view.w, Math.round(self.view.w
          * (self.view.w / self.page.w)));
      self.cursorwidth = (self.opt.cursorfixedheight) ? self.opt.cursorfixedheight
          : Math.max(self.opt.cursorminheight, self.cursorwidth);

      self.scrollvaluemax = self.view.h - self.cursorheight
          - self.cursor.hborder;
      ...
      if (sy > self.page.maxh) {
        ...
        self.scroll.y = Math.round(self.getScrollTop()
            * (1 / self.scrollratio.y));
        self.scroll.x = Math.round(self.getScrollLeft()
            * (1 / self.scrollratio.x));
    ...
    this.bind = function(dom, name, fn, bubble) { // touch-oriented & fixing
      // jquery bind
      ...
      if (name == 'mousewheel') {
        if ("onwheel" in self.win) {
          ...
          var wname = (typeof document.onmousewheel != "undefined") ? "mousewheel"
              : "DOMMouseScroll"; // older IE/Firefox
          _modernWheelEvent(el, wname, fn, bubble || false);
          if (wname == "DOMMouseScroll")
            _modernWheelEvent(el, "MozMousePixelScroll", fn, bubble || false); // Firefox
          // legacy
        }
      } else if (el.addEventListener) {
        if (cap.cantouch && /mouseup|mousedown|mousemove/.test(name)) { // touch
          // device
          // support
          var tt = (name == 'mousedown') ? 'touchstart'
        ...
        self._bind(el, name, function(e) {
          ...
          return ((fn.call(el, e) === false) || bubble === false) ? self
              .cancelEvent(e) : true;
        });
      }
    };
    ...
    this.showRailHr = function() {
      ...
      if ((self.page.maxw != 0)
        ...
        self.railh.stop().fadeIn();
      }
      return self;
    };

    this.hideRail = function(i) {
      self.visibility = false;
      self.rail.visibility = false;
      self.rail.stop().fadeOut(i);
      return self;
    };

    this.hideRailHr = function(i) {
      if (!self.railh)
        return self;
      self.railh.visibility = false;
      self.railh.stop().fadeOut(i);
      return self;
    };
    ...
    this.hide = function(i) {
      ...
      self.hidden = true;
      self.locked = true;
      return self.hideRail(i).hideRailHr(i);
    };
    ...
    this.isScrollable = function(e) {
      ...
      while (dom && (dom.nodeType == 1) && !(/BODY|HTML/.test(dom.nodeName))) {
        var dd = $(dom);
        var ov = dd.css('overflowY') || dd.css('overflowX')
            || dd.css('overflow') || '';
    ...
    function execScrollWheel(e, hr, chkscroll) {
      ...
      if (hr && (px == 0) && py) { // classic vertical-only mousewheel +
        // browser with x/y support
        px = py;
        py = 0;
      }
      ...
      if (py) {
        if (self.opt.nativeparentscrolling && chkscroll && !self.ispage
            && !self.zoomactive) {
          if (py < 0) {
    ...
    this.stop = function() {
      ...
      self.scroll.y = Math
          .round(self.getScrollTop() * (1 / self.scrollratio.y));
      self.noticeCursor();
      return self;
    };
    ...
    if (!self.opt.smoothscroll) {
      ...
      this.prepareTransition = function(dif, istime) {
        var ex = (istime) ? ((dif > 20) ? dif : 0) : self
            .getTransitionSpeed(dif);
      ...
      this.doScrollPos = function(x, y, spd) { // trans
        ...
        self.timer = setTimeout(
            function() {
              ...
              var dd = Math.round(Math.sqrt(Math.pow(dst.x, 2)
                  + Math.pow(dst.y, 2)));

              // var df = (self.newscrollspeed) ? self.newscrollspeed : dd;

              var ms = (self.newscrollspeed && self.newscrollspeed > 1) ? self.newscrollspeed
              ...
              if (ms > 0) {
                ...
                if (cap.transitionend) {
                  if (!self.scrollendtrapped) {
                    ...
                    self.bind(self.doc, cap.transitionend, self.onScrollEnd,
                        false); // I have got to do something usefull!!
                  }
                } else {
                  if (self.scrollendtrapped)
                    clearTimeout(self.scrollendtrapped);
                  self.scrollendtrapped = setTimeout(self.onScrollEnd, ms); // simulate
                  // transitionend
                  // event
                }
      ...
      this.doScrollPos = function(x, y, spd) { // no-trans
        var y = ((typeof y == "undefined") || (y === false)) ? self
            .getScrollTop(true) : y;
        ...
        var dst = Math.round(Math.sqrt(Math.pow(self.dst.x, 2)
            + Math.pow(self.dst.y, 2)));
        ...
        if (spd && spd <= 1)
          ms *= spd;
        if (ms > 0) {
          self.bzscroll = (self.bzscroll) ? self.bzscroll.update(pe, ms)
        ...
        function scrolling() {
          ...
          if (self.dst.ay) {
            sc = (self.bzscroll) ? self.dst.py
                + (self.bzscroll.getNow() * self.dst.ay) : self.newscrolly;
            var dr = sc - sy;
          ...
          if (self.dst.ax) {
            scx = (self.bzscroll) ? self.dst.px
                + (self.bzscroll.getNow() * self.dst.ax) : self.newscrollx;
            var dr = scx - sx;
    ...
    this.doZoomIn = function(e) {
      ...
      var lst = [
          'position', 'top', 'left', 'zIndex', 'backgroundColor', 'marginTop',
    ...
    this.doMomentum = function(gp) {
      ...
      if (sy || sx) {
        ...
        var onscroll = function() {
          ...
          if (self.speedx) {
            nx = Math.floor(self.lastscrollx
                - (self.speedx * (1 - self.demulxy)));
            self.lastscrollx = nx;
            if ((nx < 0) || (nx > pagew))
              df = 0.10;
          }
          ...
          if (self.speedy) {
            ny = Math.floor(self.lastscrolly
                - (self.speedy * (1 - self.demulxy)));
            self.lastscrolly = ny;
            if ((ny < 0) || (ny > pageh))
              df = 0.10;
          }
  ...
  jQuery.cssHooks["pageYOffset"] = {
    get : function(elem, computed, extra) {
      ...
      return (nice && nice.ishwscroll) ? nice.getScrollTop() : _scrollTop
          .call(elem);
    },
  ...
  /*
   * $.fx.step["scrollTop"] = function(fx){ $.cssHooks["scrollTop"].set(
   * fx.elem, fx.now + fx.unit ); };
   */
  ...
  $.cssHooks.pageXOffset = {
    get : function(elem, computed, extra) {
      var nice = $.data(elem, '__nicescroll') || false;
      return (nice && nice.ishwscroll) ? nice.getScrollLeft() : _scrollLeft
          .call(elem);
    },
    set : function(elem, value) {
      var nice = $.data(elem, '__nicescroll') || false;
  ...
  /*
   * $.fx.step["scrollLeft"] = function(fx){ $.cssHooks["scrollLeft"].set(
   * fx.elem, fx.now + fx.unit ); };
   */
  ...
  ;
  mplex(NiceScrollArray.prototype, [
      'show', 'hide', 'toggle', 'onResize', 'resize', 'remove', 'stop',
      'doScrollPos' ], function(e, n) {
    e[n] = function() {
      var args = arguments;
      return this.each(function() {
